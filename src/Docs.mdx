...
An `Overlay` component can be defined that covers the full swiper container above the slides. Common use cases involve adding controls and gradient fade effects. The component must return an element (not `null`). The props given to the component are:
